...
const firebaseConfig = {
  apiKey: "AIzaSyBKE-26b9-BYbFObxVo9ydRBL1qIrVxdLY",
  authDomain: "artsywishh-a77c5.firebaseapp.com",
  projectId: "artsywishh-a77c5",
  storageBucket: "artsywishh-a77c5.firebasestorage.app",
  messagingSenderId: "414725651161",
  appId: "1:414725651161:web:e67e2c115fbc8e0b0a1cc8",
};

const app = initializeApp(firebaseConfig);
const db = getFirestore(app);

const CLOUDINARY_CLOUD = "divzsy0gp";
const CLOUDINARY_PRESET = "artsywishh";
const WHATSAPP_NUMBER = "919999951933";
const CATEGORIES = ["All", "Bedsheets", "Sarees", "Artwork"];

const SEED_PRODUCTS = [
  { name: "Ivory Bloom Bedsheet Set", category: "Bedsheets", price: 1299, originalPrice: 1799, emoji: "🌸", color: "#f9e8f0", tag: "Bestseller", description: "100% cotton, 300 thread count with delicate floral embroidery. Includes 1 bedsheet + 2 pillowcases.", stock: 12, image: "https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400&q=80" },
  { name: "Rose Petal Duvet Cover", category: "Bedsheets", price: 1599, originalPrice: 2100, emoji: "🌹", color: "#fde8e8", tag: "New", description: "Soft microfibre with botanical print. Available in Queen & King sizes.", stock: 8, image: "https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=400&q=80" },
  { name: "Lavender Dreams Set", category: "Bedsheets", price: 999, originalPrice: 1399, emoji: "💜", color: "#ede8fd", tag: "Sale", description: "Hypoallergenic cotton blend. Perfect for sensitive skin.", stock: 20, image: "https://images.unsplash.com/photo-1560185127-6ed189bf02f4?w=400&q=80" },
  { name: "Kanjivaram Silk Saree", category: "Sarees", price: 8999, originalPrice: 11000, emoji: "✨", color: "#fff3e0", tag: "Premium", description: "Authentic Kanjivaram silk with zari border. Perfect for weddings and festive occasions.", stock: 3, image: "https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=400&q=80" },
  { name: "Chiffon Blush Saree", category: "Sarees", price: 2499, originalPrice: 3200, emoji: "🌷", color: "#fce4ec", tag: "Trending", description: "Lightweight chiffon in soft blush with delicate sequin work.", stock: 7, image: "https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?w=400&q=80" },
  { name: "Wildflower Watercolor", category: "Artwork", price: 3499, originalPrice: 4200, emoji: "🎨", color: "#e8f4fd", tag: "Original", description: "Hand-painted A3 watercolor on cold press paper. One-of-a-kind floral composition.", stock: 1, image: "https://images.unsplash.com/photo-1578301978693-85fa9c0320b9?w=400&q=80" },
  { name: "Botanical Line Art Print", category: "Artwork", price: 799, originalPrice: 999, emoji: "🌿", color: "#f1f8e9", tag: "Print", description: "Fine line botanical illustration. Giclée print on archival paper. 8x10 inches.", stock: 30, image: "https://images.unsplash.com/photo-1513519245088-0e12902e5a38?w=400&q=80" },
];
...
export default function Artsywishh() {
  ...
  const loadProducts = async () => {
    try {
      ...
      toast_("Failed to load products");
    }
    setLoading(false);
  };
  ...
  const saveNew = async () => {
    ...
    const data = { ...newProd, price: +newProd.price, originalPrice: +(newProd.originalPrice || newProd.price), stock: +(newProd.stock || 10) };
    const ref = await addDoc(collection(db, "products"), data);
    setProducts(p => [...p, { id: ref.id, ...data }]);
  ...
  const saveEdit = async () => {
    const { id, ...data } = editProd;
    await updateDoc(doc(db, "products", id), data);
    setProducts(p => p.map(x => x.id === id ? { ...editProd } : x));
    setEditProd(null);
    toast_("Updated ✓");
  };
    ...
    <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", background: "#fdf8f5", minHeight: "100vh", color: "#3d2c2c" }}>
      ...
      <header style={{ background: "white", borderBottom: "1px solid #f5e6e8", padding: "0 16px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 20px rgba(232,180,184,.12)" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={() => setAdminOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#c4738a" }}>⚙️</button>
      ...
      <div style={{ background: "linear-gradient(135deg,#fdf0f3 0%,#fdf8f5 50%,#f5f0fd 100%)", padding: "72px 32px 56px", textAlign: "center", position: "relative", overflow: "hidden", borderBottom: "1px solid #f5e6e8" }}>
        ...
        <h1 style={{ fontSize: "clamp(38px,6vw,66px)", fontWeight: 300, color: "#3d2c2c", lineHeight: 1.12, marginBottom: 18 }}>
          Where Every Thread<br /><em style={{ color: "#c4738a" }}>Tells a Story</em>
        </h1>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <button className="btn btn-wa" onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hi Artsywishh! 🌸")}`, "_blank")}>💬 WhatsApp Us</button>
        </div>
      ...
      <div id="shop" style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 24px" }}>
        ...
        {loading
          ? <div style={{ textAlign: "center", padding: "72px 0", color: "#c0a0a0" }}>
              <div style={{ fontSize: 40, marginBottom: 14 }}>🌸</div>
              <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 14 }}>Loading products...</p>
            </div>
          : <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 24 }}>
                {filtered.map((p, i) => (
                  <div key={p.id} className="card-hover anim-fade" onClick={() => setSelProd(p)} style={{ background: "white", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 22px rgba(180,100,110,.08)", animationDelay: `${i * 0.04}s`, opacity: 0 }}>
                    <div style={{ height: 190, position: "relative", overflow: "hidden", background: p.color || "#fce8f0" }}>
                    ...
                    <div style={{ padding: "16px 18px 20px" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                        <div>
                          ...
                          {p.originalPrice > p.price && <span style={{ fontFamily: "'Jost',sans-serif", fontSize: 12, color: "#c0a0a0", textDecoration: "line-through", marginLeft: 7 }}>₹{p.originalPrice.toLocaleString()}</span>}
                        </div>
      ...
      <div id="about" style={{ background: "linear-gradient(135deg,#fdf0f3,#fdf8f5)", padding: "56px 24px", borderTop: "1px solid #f5e6e8" }}>
        <div style={{ maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 18, fontWeight: 300, lineHeight: 2, color: "#7d5c5c", marginBottom: 28 }}>
            ...
            Every piece is hand-selected for its quality, artistry, and the feeling it brings to your space.
          </p>
      ...
      {selProd && (
        <>
          ...
          <div className="modal anim-pop">
            ...
            <div style={{ background: selProd.color || "#fce8f0", borderRadius: 16, height: 200, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24, overflow: "hidden", position: "relative" }}>
              {selProd.image && <img src={selProd.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", position: "absolute" }} onError={e => e.target.style.display = "none"} />}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              ...
              {selProd.originalPrice > selProd.price && <span style={{ fontFamily: "'Jost',sans-serif", fontSize: 14, color: "#c0a0a0", textDecoration: "line-through" }}>₹{selProd.originalPrice.toLocaleString()}</span>}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 13, marginBottom: 22 }}>
                    {cart.map(item => (
                      <div key={item.id} style={{ display: "flex", gap: 12, background: "white", borderRadius: 14, padding: 13, boxShadow: "0 2px 12px rgba(180,100,110,.06)" }}>
                        <div style={{ background: item.color || "#fce8f0", borderRadius: 10, width: 52, height: 52, flexShrink: 0, overflow: "hidden" }}>
                          {item.image && <img src={item.image} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => e.target.style.display = "none"} alt="" />}
                        </div>
                        <div style={{ flex: 1 }}>
              ...
              : <>
                  ...
                  {adminTab === "products" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {products.map(p => (
                        <div key={p.id} style={{ background: "white", borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 2px 8px rgba(0,0,0,.04)" }}>
                          <div style={{ width: 38, height: 38, background: p.color || "#fce8f0", borderRadius: 8, flexShrink: 0, overflow: "hidden" }}>
                            {p.image && <img src={p.image} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => e.target.style.display = "none"} alt="" />}
                          </div>
                          ...
                          <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                            <button className="btn-sm-ok" onClick={() => toggleStock(p)}>{p.stock > 0 ? "✓" : "✗ OOS"}</button>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          <label htmlFor="new-img-upload" style={{ background: uploading ? "#ccc" : "linear-gradient(135deg,#e8b4b8,#d4899a)", color: "white", padding: "9px 18px", borderRadius: 50, fontFamily: "'Jost',sans-serif", fontSize: 12, fontWeight: 500, cursor: uploading ? "not-allowed" : "pointer" }}>
                        ...
                        {newProd.image && <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 10, color: "#7abd7a", marginTop: 5 }}>✅ Image uploaded</p>}
                      </div>
      ...
      {editProd && (
        <>
          ...
          <div className="modal anim-pop" style={{ maxWidth: 460 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div><label style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, color: "#9a7070", display: "block", marginBottom: 4 }}>NAME</label><input value={editProd.name || ""} onChange={e => setEditProd(p => ({ ...p, name: e.target.value }))} /></div>
              <div>
                <label style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, color: "#9a7070", display: "block", marginBottom: 6 }}>REPLACE IMAGE</label>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <label htmlFor="edit-img-upload" style={{ background: uploading ? "#ccc" : "linear-gradient(135deg,#e8b4b8,#d4899a)", color: "white", padding: "9px 18px", borderRadius: 50, fontFamily: "'Jost',sans-serif", fontSize: 12, fontWeight: 500, cursor: uploading ? "not-allowed" : "pointer" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[["PRICE ₹", "price"], ["ORIGINAL ₹", "originalPrice"]].map(([l, k]) => (
                  <div key={k}><label style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, color: "#9a7070", display: "block", marginBottom: 4 }}>{l}</label><input type="number" value={editProd[k] || ""} onChange={e => setEditProd(p => ({ ...p, [k]: +e.target.value }))} /></div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[["STOCK", "stock"], ["TAG", "tag"]].map(([l, k]) => (
                  <div key={k}><label style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, color: "#9a7070", display: "block", marginBottom: 4 }}>{l}</label><input value={editProd[k] || ""} onChange={e => setEditProd(p => ({ ...p, [k]: k === "stock" ? +e.target.value : e.target.value }))} /></div>
                ))}
              </div>
              <div><label style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, color: "#9a7070", display: "block", marginBottom: 4 }}>DESCRIPTION</label><textarea rows={3} value={editProd.description || ""} onChange={e => setEditProd(p => ({ ...p, description: e.target.value }))} style={{ resize: "none" }} /></div>
      ...
      <button className="btn btn-wa" onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hi Artsywishh! 🌸")}`, "_blank")}
        style={{ position: "fixed", bottom: 24, right: 24, borderRadius: "50%", width: 54, height: 54, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, boxShadow: "0 6px 24px rgba(37,211,102,.4)", zIndex: 49, padding: 0 }}>
        💬
      </button>
    </div>
  );
